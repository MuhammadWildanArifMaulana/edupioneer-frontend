...
export default function AkunGuru() {
    ...
    const fetchProfile = async () => {
      try {
        if (user?.id) {
          ...
          setProfile({
            name: userData.name || user?.name || "Guru",
            email: userData.email || user?.email || "",
            status: userData.role || user?.role || "Guru",
  ...
  const handleSave = async (updatedProfile, avatarFile) => {
    try {
      ...
      setMode("view");
      // update auth context so navbar and other parts reflect new user
