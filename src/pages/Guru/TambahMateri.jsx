...
import { materiApi, guruApi } from "../../api/index";
...
export default function TambahMateri() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await materiApi.create(formData);
      addToast("Materi berhasil ditambahkan!", { type: "success" });
      setTimeout(() => navigate(`/kelas-guru/${id}`), 1500);
    } catch (err) {
      console.error("Create materi error:", err);
      addToast("Gagal membuat materi", { type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 w-full flex justify-center">
      <Card variant="blue" size="lg" className="w-full max-w-[1300px]">
        <h2 className="text-xl font-bold mb-6">Tambah Materi</h2>
        ...
        {/* Upload */}
        <label htmlFor="materi-file" className="font-semibold text-base">
          Upload File
        </label>
        <label
          htmlFor="materi-file"
        ...
        {/* BUTTONS */}
        <div className="flex justify-center sm:justify-between items-center gap-3">
          <Button
            variant="outline"
